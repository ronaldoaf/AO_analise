...
// Na Página
// http://www.totalcorner.com/match/schedule?date=20170606
...
var data_inicial=(+new Date('2017-06-06'));

data=data_inicial;

while(data < (+new Date('2017-07-29') )){
   data_formatada=dateFormat(data,'yyyymmdd') );  
...
$('.match_status:contains(Full)').parents('tr').each(function(){
   DATA.push({
      id: Number($(this).attr('data-match_id')),
   	home:$(this).find('td:eq(3) a').text(),
      away:$(this).find('td:eq(5) a').text(),  
      timestamp: (-4*60*60*1000+Number(   toTimestamp( $(this).find('td:eq(1)').text() )  ))/1000
   });
});
